...
def _dp_decode_non_overlapping_spans(starts, ends, scores, max_len, labels_inv, pred_id, u_constraint=False):
  pred_start_id, pred_end_id = pred_id
  ...
  for start, end, i in spans:
    ...
    if pred_id is not None and not((start < pred_start_id and end < pred_start_id) or (start > pred_end_id and end > pred_end_id)):
      continue
...
def srl_decode(length_list, predict_dict, srl_labels_inv, config):
    ...
    for i in range(num_sentences):
        if "srl" in predictions:
            ...
            for j, pred_id in enumerate(zip(predict_dict["pred_starts"][i][:num_preds], predict_dict["pred_ends"][i][:num_preds])):
                ...
                if arg_spans:
                    predictions["srl"][i][str([pred_id[0], pred_id[1]])] = sorted(arg_spans, key=lambda x: (x[0], x[1]))
...
class LSGNEvaluator(object):
    ...
    def evaluate(self, model, srl_model_decoder, dev_batch, use_cuda, official_stdout=False):
        model.eval()
        ...
        for i, info_dic in enumerate(self.eval_data):
            ...
            predict_dict, loss = model(word_id_tensor, char_id_tensor, length_list, info_dic, srl_hiddens)
        ...
        for pred_dic in total_srl_rels_list:
            # [{2:[[1,2,3], [2,3,4]], 3:[[1,2,3]]}]
            gold_num += len(pred_dic)
            for arg_list in pred_dic.values():
                # {2:[[1,2,3], [2,3,4]]}
                gold_num += (len(arg_list) * 2)
        for pred_dic in srl_predictions:
            predict_num += len(pred_dic)
            for arg_list in pred_dic.values():
                predict_num += (len(arg_list) * 2)

        # Exact F1
        for i, sent_dic in enumerate(total_srl_rels_list):
            flag = False
            for pred in sent_dic.keys():
                if (not flag) and (pred in srl_predictions[i]):
                    flag = True
                    correct_num += 1
                if flag:
                    for arg_tuple in sent_dic[pred]:
                        arg_span = [arg_tuple[0], arg_tuple[1]]
                        label = arg_tuple[2]
                        for pred_arg_tuple in srl_predictions[i][pred]:
                            pred_arg_span = [pred_arg_tuple[0], pred_arg_tuple[1]]
                            pred_label = pred_arg_tuple[2]
                            if arg_span == pred_arg_span:
                                correct_num += 1
                                if label == pred_label:
                                    correct_num += 1
                        # if arg_tuple in srl_predictions[i][pred]:
                        #     correct_num += 1
                flag = False
        # for pred_dic in total_srl_rels_list:
        #     # [{2:[[1,2,3], [2,3,4]], 3:[[1,2,3]]}]
        ...
        #     for arg_list in pred_dic.values():
        #         # {2:[[1,2,3], [2,3,4]]}
        #         gold_num += len(arg_list)
        # for pred_dic in srl_predictions:
        #     predict_num += len(pred_dic)
        #     for arg_list in pred_dic.values():
        #         predict_num += len(arg_list)
        #
        # for i, sent_dic in enumerate(total_srl_rels_list):
        #     flag = False
        #     for pred in sent_dic.keys():
