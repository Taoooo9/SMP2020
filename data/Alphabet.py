...
import collections
from .Common import unk_key, padding_key
class Alphabet:
    def __init__(self, max_cap, cutoff):
        print('max_cap={}, cutoff={}'.format(max_cap, cutoff))
    ...
    def initial(self, stat):
        for key in stat:
            ...
            if key == unk_key:
                print(unk_key, ':', self.from_string(unk_key))
            if key == padding_key:
                print(padding_key, ':', self.from_string(padding_key))
        self.m_b_fixed = True
