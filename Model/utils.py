...
class SoftmaxAttention(nn.Module):
    ...
    def forward(self,
        ...
        # the batch.
        if config.cosine:
            similarity_matrix = cos_attention(premise_batch, hypothesis_batch)
        else:
...
def cos_attention(premise_batch, hypothesis_batch):
    p_normalize = F.normalize(premise_batch, dim=-1)
    h_normalize = F.normalize(hypothesis_batch, dim=-1)
    e_matrix = p_normalize.bmm(h_normalize.transpose(2, 1).contiguous())
    return e_matrix
